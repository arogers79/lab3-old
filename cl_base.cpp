...
string cl_base::get_item(string path, int level){
	...
	//вытаскивает имя объекта из пути
	...
	//возвращает указатеь на корневой объект
	cl_base * cl_base::get_object(string path){
		...
		item = get_item(path,1);
			if (item!=(get_root()->get_object_name()))
				return 0;
		//если путь некорректный, то возвращает ноль
		...
		while (!item.empty()){
			...
			if (child){
				...
				return 0;
			//ищет запрошенный объект. Если отсуствует запрошенный объект - возвращает 0.
		}
		return head;
	}
